...
- `minimal.*`, `cid-references.mhtml`, `frames-nested.*`,
  `frames-cross-origin.*` — no third-party content at all (see the two
  provenance sections below, and `cid-references.mhtml` below).
...
### `cid-references.mhtml` provenance

Hand-written in this repository, not a browser capture — no producer emits
every `cid:` reference site in one document, which is exactly what this
fixture is for. It carries one 16×16 checkerboard PNG generated by the
fixture's own author (97 bytes, no third-party content) and names it from
every place an MHTML document can name a part by Content-ID: `<img src>`,
`<img srcset>`, a `<style>` element's `url()`, a `style=` attribute's
`url()`, a linked `text/css` part's own `url()`, an SVG `<image href>`, and
an `<iframe src>` — plus one deliberately unresolvable reference
(`cid:absent@archivebridge.test`).

It also reproduces the two ways a part can claim a Content-ID: the
stylesheet has *only* a synthetic `cid:` `Content-Location` (the shape every
Blink capture writes for an inlined `<style>`), while the image has only a
`Content-ID` header and the child frame has both an ID and a real
`Content-Location`.

The regression it exists for: WebKit's resource loader never attempts a
`cid:` URL, so a `cid:` reference carried into a `.webarchive` verbatim
names a resource the archive contains and never loads. Measured in a real
`WKWebView` before the fix — every `<img>` `naturalWidth: 0`, the linked
stylesheet never applied, the frame empty — and after it, all three resolve.
Its text parts are `8bit`-encoded rather than base64 specifically so the
fixture stays readable and editable.
